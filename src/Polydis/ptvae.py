...
class RnnDecoder(nn.Module):
    ...
    def forward(self, z_chd, inference, tfr, c=None):
        # z_chd: (B, z_chd_size)
        bs = z_chd.size(0)
        z_chd_hid = self.z2dec_hid(z_chd).unsqueeze(0) # (1, B, hidden_dim)
        z_chd_in = self.z2dec_in(z_chd).unsqueeze(1) # (B, 1, z_input_dim)
        if inference:
            tfr = 0.
        token = self.init_input.repeat(bs, 1).unsqueeze(1) # (B, 1, input_dim)
        recon_root = []
        recon_chroma = []
        recon_bass = []
        ...
        for t in range(int(self.num_step / 4)):
            ...
            r_chroma = self.chroma_out(chd).view(bs, 1, 12, 2).contiguous() # a binomial distribution for each pitch
            ...
            if teacher_force and not inference:
                token = c[:, t].unsqueeze(1) # take ground truth when teacher forcing
...
class TextureEncoder(nn.Module):
    ...
    def __init__(self, emb_size, hidden_dim, z_dim, num_channel=10):
        ...
        self.fc2 = nn.Linear(1000, emb_size) # convert piano-rolls to texture embeddings
    ...
    def forward(self, pr):
        # pr: (bs, 32, 128)
        bs = pr.size(0)
        pr = pr.unsqueeze(1) # the unsqueezed dim stands for the only channel of input
